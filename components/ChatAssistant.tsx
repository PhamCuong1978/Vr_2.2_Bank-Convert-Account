...
const ChatAssistant: React.FC<ChatAssistantProps> = ({ reportData, rawStatementContent, onUpdateTransaction, onTransactionAdd, onUndoLastChange }) => {
    ...
    return (
        <>
            ...
            {/* Chat Window */}
            <div ref={chatWindowRef} className={`fixed bottom-20 z-40 bg-white dark:bg-gray-800 rounded-xl shadow-2xl flex flex-col transition-all duration-300 ease-in-out inset-x-4 md:inset-x-auto md:right-5 md:w-full md:max-w-md ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
                {/* Header */}
